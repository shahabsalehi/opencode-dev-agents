...
import { DEFAULTS, getConfig, resolveBoolean, resolveString } from "../config.js"
...
const TEST_FRAMEWORK_PATTERNS = {
  jest: {
    packageIndicators: ['jest', '@jest/core'],
    configFiles: ['jest.config.js', 'jest.config.ts', 'jest.config.json', 'jest.config.mjs'],
    testPatterns: ['*.test.js', '*.test.ts', '*.spec.js', '*.spec.ts', '__tests__/**/*.js', '__tests__/**/*.ts']
  },
  vitest: {
  ...
  mocha: {
    packageIndicators: ['mocha', 'chai'],
    configFiles: ['.mocharc.js', '.mocharc.json', '.mocharc.yaml', 'mocha.opts'],
    testPatterns: ['test/**/*.js', 'test/**/*.ts', '*.test.js', '*.test.ts']
  },
  pytest: {
...
type TestFramework = keyof typeof TEST_FRAMEWORK_PATTERNS

function detectSourceLanguageHint(sourceFiles: string[]): "javascript" | "python" | "go" | "rust" | "unknown" {
  let js = 0
  let py = 0
  let go = 0
  let rust = 0
  ...
  for (const file of sourceFiles) {
    ...
    if (!ext) continue
    if (["js", "jsx", "ts", "tsx", "mjs", "cjs"].includes(ext)) js++
    else if (ext === "py") py++
    else if (ext === "go") go++
    else if (ext === "rs") rust++
  }

  const pairs: Array<["javascript" | "python" | "go" | "rust", number]> = [
    ["javascript", js],
    ["python", py],
    ["go", go],
    ["rust", rust]
  ...
  if (pairs[0][1] === 0) return "unknown"
  return pairs[0][0]
}

async function detectTestFramework(baseDir: string, sourceFiles: string[]): Promise<TestFramework> {
  const packageJsonPath = resolve(baseDir, 'package.json')
  const pyprojectPath = resolve(baseDir, 'pyproject.toml')
  const cargoPath = resolve(baseDir, 'Cargo.toml')
  const goModPath = resolve(baseDir, 'go.mod')
  
  const sourceLanguageHint = detectSourceLanguageHint(sourceFiles)
  const frameworkScores: Record<TestFramework, number> = {
    jest: 0,
    vitest: 0,
    mocha: 0,
    pytest: 0,
  ...
  const frameworkPriority: TestFramework[] = ["vitest", "jest", "pytest", "go-test", "rust-test", "mocha"]
  const compatibility: Record<TestFramework, Array<typeof sourceLanguageHint>> = {
    jest: ["javascript", "unknown"],
    vitest: ["javascript", "unknown"],
    mocha: ["javascript", "unknown"],
    pytest: ["python", "unknown"],
    "go-test": ["go", "unknown"],
    "rust-test": ["rust", "unknown"]
  }
  ...
  try {
    ...
    for (const [framework, patterns] of Object.entries(TEST_FRAMEWORK_PATTERNS)) {
      for (const indicator of patterns.packageIndicators) {
        if (deps[indicator]) {
          frameworkScores[framework as TestFramework] += 20
        }
      }

      if (framework === "vitest" && testScript.includes("vitest")) frameworkScores.vitest += 12
      if (framework === "jest" && testScript.includes("jest")) frameworkScores.jest += 12
      if (framework === "mocha" && testScript.includes("mocha")) frameworkScores.mocha += 12
      if (framework === "pytest" && testScript.includes("pytest")) frameworkScores.pytest += 12
    }
  } catch {
    // No package.json, might be Python/Rust/Go project
  }

  for (const [framework, patterns] of Object.entries(TEST_FRAMEWORK_PATTERNS)) {
    for (const configFile of patterns.configFiles) {
      try {
        await fs.access(resolve(baseDir, configFile))
        frameworkScores[framework as TestFramework] += 8
      } catch {
  ...
  try {
    ...
    if (content.includes('[tool.pytest')) frameworkScores.pytest += 20
  } catch {
  ...
  try {
    ...
    frameworkScores['go-test'] += 20
  } catch {
    // Not a Go project
  }

  for (const framework of Object.keys(frameworkScores) as TestFramework[]) {
    if (sourceLanguageHint !== "unknown" && compatibility[framework].includes(sourceLanguageHint)) {
      frameworkScores[framework] += 6
    }
  }
  
  const sorted = (Object.entries(frameworkScores) as Array<[TestFramework, number]>).sort((a, b) => {
    if (b[1] !== a[1]) return b[1] - a[1]
    return frameworkPriority.indexOf(a[0]) - frameworkPriority.indexOf(b[0])
  })
  if (sorted[0] && sorted[0][1] > 0) {
    return sorted[0][0]
  }

  if (sourceLanguageHint === "python") return "pytest"
  if (sourceLanguageHint === "go") return "go-test"
  if (sourceLanguageHint === "rust") return "rust-test"
  return 'vitest'
}
...
export const testGenerator = tool({
  ...
  args: {
    ...
    framework: tool.schema.enum(["jest", "vitest", "mocha", "pytest", "go-test", "rust-test", "auto"]).default("auto").describe("Testing framework to use (auto-detect if not specified)"),
    ...
    diffOnly: tool.schema.boolean().default(false).describe("Generate tests only for files changed since last run")
  },
  async execute(args, context) {
    const config = getConfig().tools?.testGenerator
    const baseDir = context.directory || cwd()
    const detectedFramework = args.framework === "auto" ? await detectTestFramework(baseDir, args.sourceFiles) : args.framework
    const resolvedFramework = resolveString(
      detectedFramework,
      config?.framework,
      DEFAULTS.tools.testGenerator.framework
    )
    const resolvedDiffOnly = resolveBoolean(
      args.diffOnly,
      config?.diffOnly,
      DEFAULTS.tools.testGenerator.diffOnly
    )
    const resolvedIncludePrivate = resolveBoolean(
      args.includePrivate,
      config?.includePrivate,
      DEFAULTS.tools.testGenerator.includePrivate
    )
    const { sourceFiles, outputDir } = args
    ...
    try {
      ...
      await mapWithLimit(sourceFiles, 5, async (sourceFile) => {
        ...
        try {
          ...
          const functionsKey = getFileCacheKey(filePath, stats.mtimeMs, "functions")
          const cachedFunctions = analysisCache.get(functionsKey)?.value as FunctionSignature[] | undefined
          if (cachedFunctions && resolvedDiffOnly) {
            return
          }
          ...
          // Extract functions
          const functions = cachedFunctions ?? extractFunctions(content, language)
          if (!cachedFunctions) {
            analysisCache.set(functionsKey, { value: functions, mtimeMs: stats.mtimeMs })
          }
          
          // Filter exported functions unless includePrivate is true
          const targetFunctions = resolvedIncludePrivate 
            ? functions 
      ...
      context.metadata({
        ...
        metadata: {
          ...
          functionsFound: summary.functionsFound,
          testCasesGenerated: summary.testCasesGenerated,
          filesWritten: summary.filesWritten,
          estimatedCoverage: summary.estimatedCoverage
        }
      })
      ...
      output += `### Summary\n`
      output += `- **Framework:** ${resolvedFramework}${args.framework === "auto" ? " (auto-detected)" : ""}\n`
      output += `- **Files Analyzed:** ${summary.filesAnalyzed}\n`
      output += `- **Mode:** ${resolvedDiffOnly ? "Diff-only" : "Full"}\n`

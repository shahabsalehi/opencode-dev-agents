...
import { DEFAULTS, getConfig, resolveBoolean, resolveNumber } from "../config.js"
...
export const dependencyGraph = tool({
  ...
  args: {
    ...
    diffOnly: tool.schema.boolean().default(false).describe("Include only files changed since last run")
  },
  async execute(args, context) {
    const config = getConfig().tools?.dependencyGraph
    const resolvedDepth = resolveNumber(
      args.depth,
      config?.depth,
      DEFAULTS.tools.dependencyGraph.depth
    )
    const resolvedDiffOnly = resolveBoolean(
      args.diffOnly,
      config?.diffOnly,
      DEFAULTS.tools.dependencyGraph.diffOnly
    )
    const resolvedMaxFiles = config?.maxFiles ?? DEFAULTS.tools.dependencyGraph.maxFiles
    const { entryPoints, direction } = args
    ...
    try {
      ...
      const processNode = async (currentPath: string, currentDepth: number) => {
        if (nodes[currentPath]) return
        if (currentDepth > resolvedDepth) return

        try {
          const stats = await fs.stat(currentPath)
          const contentKey = getFileCacheKey(currentPath, stats.mtimeMs, "content")
          const cachedContent = fileContentCache.get(contentKey)?.value
          const content = cachedContent ?? await fs.readFile(currentPath, 'utf-8')
          if (!cachedContent) {
            fileContentCache.set(contentKey, { value: content, mtimeMs: stats.mtimeMs })
          }

          const language = detectLanguage(currentPath)
          const parseKey = getFileCacheKey(currentPath, stats.mtimeMs, "parse")
          const cachedParse = analysisCache.get(parseKey)?.value as { imports: ImportInfo[]; exports: ExportInfo[] } | undefined
          const imports = cachedParse?.imports ?? (language ? parseImports(content, currentPath, language) : [])
          const exports = cachedParse?.exports ?? (language ? parseExports(content, language) : [])
          if (!cachedParse) {
            analysisCache.set(parseKey, { value: { imports, exports }, mtimeMs: stats.mtimeMs })
          }
          ...
          for (const imp of imports) {
            ...
            if (resolved) {
              ...
              if (currentDepth < resolvedDepth && direction !== "in") {
                queue.push({ path: resolved, depth: currentDepth + 1 })
              }
            }
          }

          const graphKey = getFileCacheKey(currentPath, stats.mtimeMs, "graph")
          const graphCached = analysisCache.get(graphKey)
          if (!resolvedDiffOnly || !graphCached) {
            nodes[currentPath] = {
              path: currentPath,
              imports,
              exports,
            ...
            analysisCache.set(graphKey, { value: true, mtimeMs: stats.mtimeMs })
            processedCount++
          }
        } catch (error) {
      ...
      while (queue.length > 0 && processedCount < maxFiles) {
        ...
        await mapWithLimit(batch, 8, async ({ path: currentPath, depth: currentDepth }) => {
          if (processedCount >= maxFiles) return
          await processNode(currentPath, currentDepth)
        })
      }
      ...
      // Calculate max depth
      const maxDepth = Object.values(nodes).length > 0
        ? Math.max(...Object.values(nodes).map(n => n.depth))
        : 0
      ...
      context.metadata({
        ...
        metadata: {
          ...
          circularDependencyCount: summary.circularDependencyCount,

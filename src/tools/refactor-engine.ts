...
import { refactorCache, fileContentCache, getFileCacheKey } from "../utils/cache.js"
...
function validateChanges(original: string, modified: string, extension: string): ValidationResult {
  const errors: string[] = []
  const warnings: string[] = []

  if ([".ts", ".tsx", ".js", ".jsx"].includes(extension)) {
    const diagnostics = validateTypeScript(modified)
...
function validateTypeScript(source: string): string[] {
  const result = ts.transpileModule(source, {
    compilerOptions: { allowJs: true },
    reportDiagnostics: true
  })
  const diagnostics = result.diagnostics ?? []
  return diagnostics.map((diag) => ts.flattenDiagnosticMessageText(diag.messageText, " "))
}

export const refactorEngine = tool({
  description: "Performs safe code refactoring transformations. Supports modernizing syntax, extracting functions, renaming symbols, removing dead code, and optimizing imports with validation. Use mode=\"lint\" for fast, heuristic-only refactors, or mode=\"precision\" for stricter checks.",
  args: {
    ...
    diffOnly: tool.schema.boolean().default(false).describe("Refactor only files changed since last run"),
    mode: tool.schema.enum(["lint", "precision"]).default("lint").describe("Refactor mode: lint (fast) or precision (stricter validation)")
  },
  async execute(args, context) {
    const config = getConfig().tools?.refactorEngine
    const resolvedMode = resolveString(
      args.mode,
      config?.mode,
      DEFAULTS.tools.refactorEngine.mode
    ) as "lint" | "precision"
    const resolvedDryRun = resolveBoolean(
      args.dryRun,
      config?.dryRun,
      DEFAULTS.tools.refactorEngine.dryRun
    )
    const resolvedDiffOnly = resolveBoolean(
      args.diffOnly,
      config?.diffOnly,
      DEFAULTS.tools.refactorEngine.diffOnly
    )
    const { files, transformation, options = {}, preview } = args
    ...
    try {
      ...
      await mapWithLimit(files, 6, async (file) => {
        ...
        try {
          ...
          const refactorKey = getFileCacheKey(filePath, stats.mtimeMs, transformation)
          const cachedResult = refactorCache.get(refactorKey)?.value as {
            changes: CodeChange[]
            validation: ValidationResult
          } | undefined
          if (cachedResult) {
            if (!resolvedDiffOnly) {
              results.push({
                ...
                changes: preview ? cachedResult.changes : [],
                validation: cachedResult.validation,
                applied: false
              })
            }
            return
          ...
          const validation = resolvedMode === "precision"
            ? validateChanges(content, modifiedContent, ext)
            : { valid: true, errors: [], warnings: [] }
          
          // Apply changes if validation passes and not dry run
          let applied = false
          if (!resolvedDryRun && validation.valid) {
          ...
          refactorCache.set(refactorKey, { value: { changes, validation }, mtimeMs: stats.mtimeMs })
      ...
      output += `- **Mode:** ${resolvedDryRun ? 'Dry Run (no changes applied)' : 'Live'}\n`
      output += `- **Scope:** ${resolvedDiffOnly ? 'Diff-only' : 'Full'}\n`
      output += `- **Validation:** ${resolvedMode === 'precision' ? 'Precision' : 'Lint'}\n\n`
